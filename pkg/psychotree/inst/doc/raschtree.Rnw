...
\Address{
  ...
  E-mail: \email{Carolin.Strobl@stat.uni-muenchen.de}, \email{Julia.Kopf@stat.uni-muenchen.de}\\
...
model fitting. These do not contribute anything to the Rasch model because they do not contain any information which items are easier and which are
more difficult. Thus, these observations are irrelevant for the model and the corresponding person parameters are not identified.
...
Then we select the subset of cases for which the proportion of correct item responses is strictly between 0 and 1.
%
...
@
%
<<fit_raschtree_cache, echo=FALSE>>=
...
colors \citep[see][]{Zeietal:2009} is employed to indicate the blocks of nine items
...
Here the item parameters have been displayed only for the subjects in node number 4 (representing female students who access the online magazine up to once
per week and are up to 21~years of age) to save space. The item parameters for all groups can be displayed by omitting the \code{node} argument.
